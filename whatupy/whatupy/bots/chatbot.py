...
class ChatBot(BaseBot):
    ...
    async def start(self, **kwargs):
        self.strAnonJID = utils.jid_to_str(
            (
    ...
    async def initiate_friend_chats(self, *args, **kwargs):
        ...
        for anon_friend in CHATBOT_FRIEND_JIDS:
            if anon_friend == self.strAnonJID:
                continue
            self.logger.info(f"Saying hello to: {CHATBOT_FRIEND_JIDS[anon_friend]}")
            await asyncio.sleep(5)
            await self.send_text_message(
                CHATBOT_FRIEND_JIDS[anon_friend], self.generate_message()
            )
    ...
    async def on_message(self, message: wuc.WUMessage, **kwargs):
        ...
        self.logger.info(
            f"Sending message in {wait_time}seconds: {anon_chat_JID}: {response}"
        )
        ...
        status = await self.send_text_message(
            CHATBOT_FRIEND_JIDS[anon_chat_JID], response
        )
