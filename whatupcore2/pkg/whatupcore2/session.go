...
func NewSessionLogin(username string, passphrase string, secretKey []byte, opts *WhatsAppClientConfig, log waLog.Logger) (*Session, error) {
	session, err := NewSessionDisconnected(username, passphrase, secretKey, log)
	if err != nil {
		return nil, err
	}

	if err = session.Login(username, passphrase, opts); err != nil {
...
func NewSessionRegister(ctx context.Context, username string, passphrase string, secretKey []byte, registerOptions *pb.RegisterOptions, opts *WhatsAppClientConfig, log waLog.Logger) (*Session, *RegistrationState, error) {
	session, err := NewSessionDisconnected(username, passphrase, secretKey, log)
	if err != nil {
		return nil, nil, err
	}

	state, err := session.LoginOrRegister(ctx, username, passphrase, registerOptions, opts)
	if err != nil {
		return nil, nil, err
	}
...
func (session *Session) LoginOrRegister(ctx context.Context, username string, passphrase string, registerOptions *pb.RegisterOptions, opts *WhatsAppClientConfig) (*RegistrationState, error) {
	client, err := NewWhatsAppClient(session.ctxC, username, passphrase, opts, session.log.Sub("WAC"))
	if err != nil {
		return nil, err
	}
