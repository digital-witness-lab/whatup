...
// NewEncSQLStore creates a new EncSQLStore with the given database container and user JID.
...
var _ store.IdentityStore = (*EncSQLStore)(nil)
var _ store.SessionStore = (*EncSQLStore)(nil)
var _ store.PreKeyStore = (*EncSQLStore)(nil)
var _ store.SenderKeyStore = (*EncSQLStore)(nil)
var _ store.AppStateSyncKeyStore = (*EncSQLStore)(nil)
var _ store.AppStateStore = (*EncSQLStore)(nil)
var _ store.ContactStore = (*EncSQLStore)(nil)

const (
	putIdentityQuery = `
		INSERT INTO whatsmeow_enc_identity_keys (our_jid, their_id, identity) VALUES ($1, $2, $3)
		ON CONFLICT (our_jid, their_id) DO UPDATE SET identity=excluded.identity
	`
	deleteAllIdentitiesQuery = `DELETE FROM whatsmeow_enc_identity_keys WHERE our_jid=$1 AND their_id LIKE $2`
	deleteIdentityQuery      = `DELETE FROM whatsmeow_enc_identity_keys WHERE our_jid=$1 AND their_id=$2`
	getIdentityQuery         = `SELECT identity FROM whatsmeow_enc_identity_keys WHERE our_jid=$1 AND their_id=$2`
)
...
const (
	getSessionQuery = `SELECT session FROM whatsmeow_enc_sessions WHERE our_jid=$1 AND their_id=$2`
	hasSessionQuery = `SELECT true FROM whatsmeow_enc_sessions WHERE our_jid=$1 AND their_id=$2`
	putSessionQuery = `
		INSERT INTO whatsmeow_enc_sessions (our_jid, their_id, session) VALUES ($1, $2, $3)
		ON CONFLICT (our_jid, their_id) DO UPDATE SET session=excluded.session
	`
	deleteAllSessionsQuery = `DELETE FROM whatsmeow_enc_sessions WHERE our_jid=$1 AND their_id LIKE $2`
	deleteSessionQuery     = `DELETE FROM whatsmeow_enc_sessions WHERE our_jid=$1 AND their_id=$2`
)
...
const (
	getLastPreKeyIDQuery        = `SELECT MAX(key_id) FROM whatsmeow_enc_pre_keys WHERE jid=$1`
	insertPreKeyQuery           = `INSERT INTO whatsmeow_enc_pre_keys (jid, key_id, key, uploaded) VALUES ($1, $2, $3, $4)`
	getUnuploadedPreKeysQuery   = `SELECT key_id, key FROM whatsmeow_enc_pre_keys WHERE jid=$1 AND uploaded=false ORDER BY key_id LIMIT $2`
	getPreKeyQuery              = `SELECT key_id, key FROM whatsmeow_enc_pre_keys WHERE jid=$1 AND key_id=$2`
	deletePreKeyQuery           = `DELETE FROM whatsmeow_enc_pre_keys WHERE jid=$1 AND key_id=$2`
	markPreKeysAsUploadedQuery  = `UPDATE whatsmeow_enc_pre_keys SET uploaded=true WHERE jid=$1 AND key_id<=$2`
	getUploadedPreKeyCountQuery = `SELECT COUNT(*) FROM whatsmeow_enc_pre_keys WHERE jid=$1 AND uploaded=true`
)
...
func (s *EncSQLStore) GenOnePreKey() (*keys.PreKey, error) {
	s.preKeyLock.Lock()
...
func (s *EncSQLStore) GetOrGenPreKeys(count uint32) ([]*keys.PreKey, error) {
	s.preKeyLock.Lock()
	defer s.preKeyLock.Unlock()
...
func (s *EncSQLStore) scanPreKey(row scannable) (*keys.PreKey, error) {
	var priv []byte
	var id uint32
	err := decryptDBScan(s, row, &id, &priv)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
const (
	getSenderKeyQuery = `SELECT sender_key FROM whatsmeow_enc_sender_keys WHERE our_jid=$1 AND chat_id=$2 AND sender_id=$3`
	putSenderKeyQuery = `
		INSERT INTO whatsmeow_enc_sender_keys (our_jid, chat_id, sender_id, sender_key) VALUES ($1, $2, $3, $4)
...
const (
	putAppStateSyncKeyQuery = `
		INSERT INTO whatsmeow_enc_app_state_sync_keys (jid, key_id, key_data, timestamp, fingerprint) VALUES ($1, $2, $3, $4, $5)
		ON CONFLICT (jid, key_id) DO UPDATE
			...
			WHERE excluded.timestamp > whatsmeow_enc_app_state_sync_keys.timestamp
	`
	getAppStateSyncKeyQuery         = `SELECT key_data, timestamp, fingerprint FROM whatsmeow_enc_app_state_sync_keys WHERE jid=$1 AND key_id=$2`
	getLatestAppStateSyncKeyIDQuery = `SELECT key_id FROM whatsmeow_enc_app_state_sync_keys WHERE jid=$1 ORDER BY timestamp DESC LIMIT 1`
)
...
const (
	putAppStateVersionQuery = `
		INSERT INTO whatsmeow_enc_app_state_version (jid, name, version, hash) VALUES ($1, $2, $3, $4)
		ON CONFLICT (jid, name) DO UPDATE SET version=excluded.version, hash=excluded.hash
	`
	getAppStateVersionQuery                 = `SELECT version, hash FROM whatsmeow_enc_app_state_version WHERE jid=$1 AND name=$2`
	deleteAppStateVersionQuery              = `DELETE FROM whatsmeow_enc_app_state_version WHERE jid=$1 AND name=$2`
	putAppStateMutationMACsQuery            = `INSERT INTO whatsmeow_enc_app_state_mutation_macs (jid, name, version, index_mac, value_mac) VALUES `
	deleteAppStateMutationMACsQueryPostgres = `DELETE FROM whatsmeow_enc_app_state_mutation_macs WHERE jid=$1 AND name=$2 AND index_mac=ANY($3::bytea[])`
	deleteAppStateMutationMACsQueryGeneric  = `DELETE FROM whatsmeow_enc_app_state_mutation_macs WHERE jid=$1 AND name=$2 AND index_mac IN `
	getAppStateMutationMACQuery             = `SELECT value_mac FROM whatsmeow_enc_app_state_mutation_macs WHERE jid=$1 AND name=$2 AND index_mac=$3 ORDER BY version DESC LIMIT 1`
)
...
func (s *EncSQLStore) putAppStateMutationMACs(tx execable, name string, version uint64, mutations []store.AppStateMutationMAC) error {
...
func (s *EncSQLStore) PutAppStateMutationMACs(name string, version uint64, mutations []store.AppStateMutationMAC) error {
...
func (s *EncSQLStore) DeleteAppStateMutationMACs(name string, indexMACs [][]byte) (err error) {
	if len(indexMACs) == 0 {
		return
	}
...
const (
	putContactNameQuery = `
		INSERT INTO whatsmeow_enc_contacts (our_jid, their_jid, first_name, full_name) VALUES ($1, $2, $3, $4)
	...
	putManyContactNamesQuery = `
		INSERT INTO whatsmeow_enc_contacts (our_jid, their_jid, first_name, full_name)
		VALUES %s
		ON CONFLICT (our_jid, their_jid) DO UPDATE SET first_name=excluded.first_name, full_name=excluded.full_name
	`
	putPushNameQuery = `
		INSERT INTO whatsmeow_enc_contacts (our_jid, their_jid, push_name) VALUES ($1, $2, $3)
		ON CONFLICT (our_jid, their_jid) DO UPDATE SET push_name=excluded.push_name
	`
	putBusinessNameQuery = `
		INSERT INTO whatsmeow_enc_contacts (our_jid, their_jid, business_name) VALUES ($1, $2, $3)
		ON CONFLICT (our_jid, their_jid) DO UPDATE SET business_name=excluded.business_name
	`
	getContactQuery = `
		SELECT first_name, full_name, push_name, business_name FROM whatsmeow_enc_contacts WHERE our_jid=$1 AND their_jid=$2
	`
	getAllContactsQuery = `
		SELECT their_jid, first_name, full_name, push_name, business_name FROM whatsmeow_enc_contacts WHERE our_jid=$1
	`
)

func (s *EncSQLStore) PutPushName(user types.JID, pushName string) (bool, string, error) {
...
func (s *EncSQLStore) PutBusinessName(user types.JID, businessName string) (bool, string, error) {
	s.contactCacheLock.Lock()
	defer s.contactCacheLock.Unlock()
...
func (s *EncSQLStore) PutContactName(user types.JID, firstName, fullName string) error {
	s.contactCacheLock.Lock()
	defer s.contactCacheLock.Unlock()
...
func (s *EncSQLStore) putContactNamesBatch(tx execable, contacts []store.ContactEntry) error {
...
func (s *EncSQLStore) PutAllContactNames(contacts []store.ContactEntry) error {
...
func (s *EncSQLStore) getContact(user types.JID) (*types.ContactInfo, error) {
...
func (s *EncSQLStore) GetAllContacts() (map[types.JID]types.ContactInfo, error) {
	...
	for rows.Next() {
		var jid types.JID
		var first, full, push, business sql.NullString
		err = decryptDBScan(s, rows, &jid, &first, &full, &push, &business)
		if err != nil {
...
const (
	putChatSettingQuery = `
		INSERT INTO whatsmeow_enc_chat_settings (our_jid, chat_jid, %[1]s) VALUES ($1, $2, $3)
		ON CONFLICT (our_jid, chat_jid) DO UPDATE SET %[1]s=excluded.%[1]s
	`
	getChatSettingsQuery = `
		SELECT muted_until, pinned, archived FROM whatsmeow_enc_chat_settings WHERE our_jid=$1 AND chat_jid=$2
	`
)

func (s *EncSQLStore) PutMutedUntil(chat types.JID, mutedUntil time.Time) error {
	var val int64
...
const (
	putMsgSecret = `
		INSERT INTO whatsmeow_enc_message_secrets (our_jid, chat_jid, sender_jid, message_id, key)
		VALUES ($1, $2, $3, $4, $5)
		ON CONFLICT (our_jid, chat_jid, sender_jid, message_id) DO NOTHING
	`
	getMsgSecret = `
		SELECT key FROM whatsmeow_enc_message_secrets WHERE our_jid=$1 AND chat_jid=$2 AND sender_jid=$3 AND message_id=$4
	`
)

func (s *EncSQLStore) PutMessageSecrets(inserts []store.MessageSecretInsert) (err error) {
	tx, err := s.db.Begin()
	if err != nil {
...
const (
	putPrivacyTokens = `
		INSERT INTO whatsmeow_enc_privacy_tokens (our_jid, their_jid, token, timestamp)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (our_jid, their_jid) DO UPDATE SET token=EXCLUDED.token, timestamp=EXCLUDED.timestamp
	`
	getPrivacyToken = `SELECT token, timestamp FROM whatsmeow_enc_privacy_tokens WHERE our_jid=$1 AND their_jid=$2`
)

func (s *EncSQLStore) PutPrivacyTokens(tokens ...store.PrivacyToken) error {
...
func (s *EncSQLStore) GetPrivacyToken(user types.JID) (*store.PrivacyToken, error) {
...
func (s *EncSQLStore) PutMessageInfo(msgInfo *types.MessageInfo) error {
	_, err := encryptQueryRows(s, s.db.Exec, putNewestMessage, s.JID, msgInfo.Chat.ToNonAD(), msgInfo.ID, msgInfo.Timestamp, msgInfo.IsFromMe)
	return err
}
